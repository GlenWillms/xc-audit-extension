...
function buildLbHtml(result, checkCategories) {
  ...
  for (const cat of result.categorized) {
    ...
    for (const d of cat.warnings) {
      const topKey = d.path.split('.')[1];
      if (!warningsByKey.has(topKey)) warningsByKey.set(topKey, []);
      warningsByKey.get(topKey).push(d);
    }
    for (const [topKey, diffs] of warningsByKey) {
      const d = diffs[0];
      ...
      if (!active) {
        ...
        if (diffs.length === 1 && d.type !== 'MISSING') {
          html += ` &mdash; expected: <code>${escapeHtml(fmt(d.expected))}</code>, found: <code>${escapeHtml(fmt(d.found))}</code>`;
        } else if (diffs.every(dd => dd.type === 'MISSING')) {
          html += isOptional ? '' : ' &mdash; missing';
        } else {
          html += ` &mdash; ${diffs.length} settings differ from baseline`;
        }
        const reason = diffs.find(dd => dd.explanation)?.explanation?.reason;
        if (reason) html += `<div class="finding-reason">${escapeHtml(reason)}</div>`;
        html += `</div>`;
      }
    }
    ...
    for (const insp of cat.inspections || []) {
      ...
      if (!active && insp.pass) {
        ...
        if (insp.diffs.length === 1) {
          const d = insp.diffs[0];
          if (d.type === 'MISSING') html += ' &mdash; missing';
          else html += ` &mdash; expected: <code>${escapeHtml(fmt(d.expected))}</code>, found: <code>${escapeHtml(fmt(d.found))}</code>`;
          if (d.explanation) html += `<div class="finding-reason">${escapeHtml(d.explanation.reason)}</div>`;
        } else {
          html += ' &mdash; missing';
          const reason = insp.diffs.find(d => d.explanation)?.explanation?.reason;
          if (reason) html += `<div class="finding-reason">${escapeHtml(reason)}</div>`;
        }
        html += `</div>`;
      }
